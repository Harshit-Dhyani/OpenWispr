"""Refiner service response caching for transcript refinement.

Provides thread-safe caching for refiner service responses to reduce redundant
...
class RefinerCache:
    ...
    def _persist(self) -> None:
        ...
        if evicted or expired:
            logger.debug(
                "RefinerCache persisted: entries=%d evicted=%d expired=%d",
                len(payload),
                evicted,
                expired,
            )
