/**
 * SnippetsPage - Trigger expansion management
 * 
 * Manages text snippet triggers and their expansions with scope filtering,
 * add/delete, preview, and JSON import/export capabilities.
 */
    ...
    >
      <div className="grid h-full min-h-0 gap-2 xl:grid-cols-[380px_minmax(0,1fr)]">
          ...
          <section className="border-2 border-lawn-border bg-lawn-panel p-2 shadow-brutal-sm">
            ...
            <div className="mt-2 grid gap-2">
              <input
                ...
                className="border-2 border-lawn-border bg-lawn-bg px-2 py-1.5 text-sm font-bold text-lawn-border outline-none"
              />
              <input
                value={expansion}
              ...
              <button
                ...
                className="border-2 border-lawn-accent bg-lawn-accent px-2 py-1.5 text-xs font-black uppercase tracking-[0.1em] text-lawn-bg"
              >
                {text.addButton}
              </button>
            </div>
          ...
          <section className="border-2 border-lawn-border bg-lawn-panel p-2 shadow-brutal-sm">
            ...
            <div className="mt-2 grid gap-2">
              ...
              <button
                ...
                className="border-2 border-lawn-border bg-lawn-dark px-2 py-1.5 text-xs font-black uppercase tracking-[0.1em] text-lawn-bg"
              >
                {text.previewButton}
              </button>
            </div>
        ...
        <section className="min-h-0 overflow-hidden border-2 border-lawn-border bg-lawn-panel shadow-brutal-sm">
          <div className="flex items-center justify-between border-b-2 border-lawn-border bg-lawn-bg p-2">
            <div>
              <h3 className="text-[10px] font-black uppercase tracking-[0.14em] text-lawn-muted">Snippet library</h3>
              <p className="mt-0.5 text-xs font-bold text-lawn-muted">{entries.length} saved snippet{entries.length === 1 ? '' : 's'}</p>
            </div>
          </div>
              ...
              {entries.map((entry) => (
                <article key={entry.id} className="flex min-w-0 items-center justify-between gap-2 border-2 border-lawn-border bg-lawn-bg p-2 transition-all duration-200 hover:border-lawn-accent/50 hover:-translate-y-0.5 hover:shadow-brutal-sm">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-black text-lawn-border break-words">{entry.trigger} → {entry.expansion}</p>
                    <p className="text-[11px] font-bold text-lawn-muted break-words">{entry.scope} · {entry.usage_count} {text.usesSuffix}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => void deleteEntry(entry.id)}
                    className="border-2 border-lawn-border bg-lawn-panel px-2 py-1 text-xs font-black uppercase tracking-[0.1em] text-lawn-border shrink-0 hover:bg-lawn-accent hover:text-lawn-bg"
                  >
                    {text.deleteButton}
                  </button>
                </article>
