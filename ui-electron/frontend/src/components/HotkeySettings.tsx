...
import type { HotkeyState, HotkeyConfig } from '../types/api';

interface HotkeySettingsProps {
  onConfigChange?: (config: HotkeyConfig) => void;
}
...
export function HotkeySettings({ onConfigChange }: HotkeySettingsProps) {
  ...
  const getStatusColor = () => {
    if (hotkeyState?.session?.is_active) return 'text-theme-success';
  ...
  const getStatusText = () => {
    if (hotkeyState?.session?.is_active) return 'Active - Currently recording';
      ...
      <div className="border-t-2 border-lawn-border pt-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          ...
          <div className="border border-lawn-border bg-lawn-bg p-3">
            ...
            <span
              className={`font-bold ${
                hotkeyState?.session?.is_active ? 'text-lawn-accent' : 'text-stone-500'
              }`}
            >
              {hotkeyState?.session?.is_active ? 'Recording' : 'Idle'}
            </span>
          </div>
