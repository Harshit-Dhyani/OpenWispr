...
interface AdvancedSettingsProps {
  isOpen: boolean;
  onClose: () => void;
  onApplySettings: (settings: OptimizedSettings) => void;
  currentSettings?: Partial<OptimizedSettings>;
}
...
const PRESETS = [
  { id: 'maximum', label: 'Maximum Quality', icon: Brain, desc: 'Best accuracy, needs 10GB+ GPU' },
  { id: 'balanced', label: 'Balanced', icon: Gauge, desc: 'Best balance for most systems' },
  { id: 'fast', label: 'Maximum Speed', icon: Zap, desc: 'Fastest transcription' },
  { id: 'low_memory', label: 'Low Memory', icon: HardDrive, desc: 'Fits in limited VRAM' },
];
...
export function AdvancedSettings({ isOpen, onClose, onApplySettings, currentSettings }: AdvancedSettingsProps) {
  ...
  const [manualSettings, setManualSettings] = useState<Partial<OptimizedSettings>>({});
  ...
  const loadOptimizedSettings = async (mode: string, hotkey = false) => {
    ...
    try {
      ...
      setManualSettings({ ...data.settings, ...currentSettings });
    } catch (error) {
      console.error('Failed to load optimized settings:', error);
      throw error;
    } finally {
      setOptimizing(false);
    }
  ...
  const handleManualChange = (key: keyof OptimizedSettings, value: unknown) => {
    setManualSettings(prev => ({ ...prev, [key]: value }));
  };

  const applyManualSettings = () => {
    if (manualSettings) {
      onApplySettings(manualSettings as OptimizedSettings);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-[800px] max-h-[90vh] bg-lawn-bg border-2 border-lawn-border shadow-brutal flex flex-col">
        ...
        <div className="flex border-b-2 border-lawn-border">
          {[
            { id: 'presets', label: 'Presets', icon: Gauge },
                ...
                <section className="mb-6 border-2 border-lawn-border bg-lawn-panel p-4">
                  ...
                  <div className="grid grid-cols-4 gap-3 text-xs">
                    <div className="border border-lawn-border bg-lawn-bg p-2">
                      <span className="text-stone-500 block">GPU</span>
                      <span className="font-bold">
                        {profile.gpu.available ? profile.gpu.name : 'Not Available'}
                      </span>
                      {profile.gpu.available && (
                        <span className="block text-stone-500">
                          {profile.gpu.vram_gb.toFixed(1)}GB VRAM
                        </span>
                      )}
                    </div>
                    <div className="border border-lawn-border bg-lawn-bg p-2">
                      <span className="text-stone-500 block">CPU</span>
                      <span className="font-bold">{profile.cpu.cores} Cores</span>
                      <span className="block text-stone-500">
                        {profile.cpu.ram_gb.toFixed(0)}GB RAM
                      </span>
                    </div>
                    <div className="border border-lawn-border bg-lawn-bg p-2">
                      <span className="text-stone-500 block">Storage</span>
                      <span className="font-bold">{profile.storage.free_gb.toFixed(0)}GB Free</span>
                      <span className="block text-stone-500">
                        {profile.storage.ssd_available ? 'SSD' : 'HDD'}
                      </span>
                    </div>
                    <div className="border border-lawn-border bg-lawn-bg p-2">
                      <span className="text-stone-500 block">Recommended</span>
                    ...
                    <div className="border-2 border-lawn-border bg-lawn-panel p-4">
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <div className="bg-lawn-bg border border-lawn-border p-3">
                          <span className="text-[10px] text-stone-500 uppercase">Quality Level</span>
                          <span className="block font-bold text-sm capitalize">
                            {optimizationResult.metadata.quality_level}
                          </span>
                        </div>
                        <div className="bg-lawn-bg border border-lawn-border p-3">
                          <span className="text-[10px] text-stone-500 uppercase">VRAM Usage</span>
                          <span className="block font-bold text-sm">
                            {optimizationResult.metadata.estimated_vram_usage_gb.toFixed(1)}GB
                          </span>
                        </div>
                        <div className="bg-lawn-bg border border-lawn-border p-3">
                          <span className="text-[10px] text-stone-500 uppercase">Latency</span>
                          <span className="block font-bold text-sm">
                            {optimizationResult.metadata.estimated_latency_ms}ms
                          </span>
                        </div>
                      </div>
                      {optimizationResult.metadata.tradeoffs.length > 0 && (
                        <div className="mt-4">
                          <span className="text-[10px] text-stone-500 uppercase">Trade-offs</span>
                      ...
                      <button
                        onClick={() => onApplySettings(optimizationResult.settings)}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full border-2 border-lawn-border bg-lawn-bg px-3 py-2 text-xs font-bold"
                        >
                          <option value="int8">INT8 (Fast, Low VRAM)</option>
                          <option value="float16">Float16 (Balanced)</option>
                          <option value="float32">Float32 (Best Quality)</option>
                        </select>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="w-4 h-4 border-2 border-lawn-border"
                        />
                        <span className="text-xs font-bold">Filter Filler Words (um, uh)</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <input
                          type="checkbox"
                          ...
                          className="w-4 h-4 border-2 border-lawn-border"
                        />
                        <span className="text-xs font-bold">Filter Hallucinations</span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="w-4 h-4 border-2 border-lawn-border"
                        />
                        <span className="text-xs font-bold">Use Parallel Processing</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <input
                          type="checkbox"
                          ...
                          className="w-4 h-4 border-2 border-lawn-border"
                        />
                        <span className="text-xs font-bold">Preload Model at Startup</span>
                      </div>
                    </div>
                  </div>
