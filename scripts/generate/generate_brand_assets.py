"""Generate brand assets - creates icons and logos for OpenWispr packaging.

Generates multi-resolution PNG icons for Linux, ICO for Windows, ICNS for macOS,
and copies assets to frontend src/assets directory. Accepts PNG, JPG, WebP, or SVG
source logos.
"""
...
def write_ico(
    image: Image.Image, destination: Path, sizes: Iterable[int], padding_ratio: float
) -> None:
    ensure_parent(destination)
    sizes = tuple(sizes)
