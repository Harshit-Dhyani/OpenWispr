...
class CommandRunner:
    ...
    def run_parallel(
        self, commands: List[tuple], max_workers: int = 4
...
class BuildOrchestrator:
            ...
            for file in dist_dir.rglob("*"):
                if file.is_file():
                    ...
                    if size > 1024 * 1024:  # Files larger than 1MB
                        self.logger.warning(
                            f"Large file: {file.name} ({size / 1024 / 1024:.2f} MB)"
                        )
    ...
    def build_production(self):
        ...
        try:
            ...
            if not (Path("node_modules").exists() and Path("app/electron/node_modules").exists()):
    ...
    def show_version(self):
        ...
        print()
        print(f"{Colors.BOLD}{Colors.HEADER}OpenWispr Build System{Colors.ENDC}")
...
def main():
    parser = argparse.ArgumentParser(
        description="OpenWispr Build System",
...
Examples:
    ...
    parser.add_argument(
        "--arch", choices=["x64", "arm64", "ia32", "all"], default="x64", help="Target architecture"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument("--no-cache", action="store_true", help="Disable build caching")
